...
class DoublyLinkedList:
    ...
    # def join(self,N):   # 현재 리스트 뒤에 list를 이어 붙임.(두 리스트 연결)
        # splice()함수 이용 OPTION
    # def split(x):     # 리스트의 노드 x부터 마지막 노드까지를 떼어내 새로운 리스트를 만들어 리턴
    # splice()함수 이용 OPTION
    def printList(self):  # 노드의 key값을 순서대로 출력하는 함수
        v = self.head
        print("h",end="")
        if self.head.next==self.head:
            print()
