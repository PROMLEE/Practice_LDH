...
#----------------------------------------------------------------------------------------------------
	...
	def pop(self, k=None): # A[k]를 제거 후 리턴. k 값이 없다면 가장 오른쪽 값 제거 후 리턴
		if self.n==0 or self.n-1 < k or -self.n > k:# 빈 리스트이거나 올바른 인덱스 범위를 벗어나면: 
			raise IndexError
		else: # 1. k 값이 주어진 경우와 주어지지 않은 경우 구별해야 함
			if k==-1:
				x=self.A[self.n-1]# 2. x = self.A[k]
				self.A[self.n-1]=None
				self.n-=1# 4. self.n -= 1
				if self.capacity >= 4 and self.n <= self.capacity//4: 
					self.change_size(self.capacity//2) # 실제 key 값이 전체의 25% 이하면 halving
				return x# 5. return x
			else:
	...
	def insert(self, k, x): # 주의: k 값이 음수값일 수도 있음
		if self.n==0 or self.n-1 < k or -self.n > k: # 1. k의 범위가 올바르지 않으면 IndexError 발생시킴
			raise IndexError
