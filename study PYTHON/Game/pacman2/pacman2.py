    ...
    for g in range(len(ghosts)):
        ...
        if inTheCentre(ghosts[g]):
            ...
            if ghosts[g].status > 200 or (ghosts[g].status > 1 and ghosts[g].status%2 == 0):
                ...
                if g == 0: followPlayer1(g, dirs)
                if g == 1: followPlayer2(g, dirs)
                if g == 2: followPlayer3(g, dirs)
                if g == 3: followPlayer4(g, dirs)
...
def followPlayer1(g, dirs):
    d = ghosts[g].dir
    if d == 1 or d == 3:
        if player.x > ghosts[g].x and dirs[0] == 1: ghosts[g].dir = 0
        if player.x < ghosts[g].x and dirs[2] == 1: ghosts[g].dir = 2
    if d == 0 or d == 2:
        if player.y+40 > ghosts[g].y and dirs[1] == 1 and not aboveCentre(ghosts[g]): ghosts[g].dir = 1
        if player.y+40 < ghosts[g].y and dirs[3] == 1: ghosts[g].dir = 3

def followPlayer2(g, dirs):
    d = ghosts[g].dir
    if d == 1 or d == 3:
        if player.x > ghosts[g].x and dirs[0] == 1: ghosts[g].dir = 0
        if player.x < ghosts[g].x and dirs[2] == 1: ghosts[g].dir = 2
...
def followPlayer3(g, dirs):
    d = ghosts[g].dir
    if d == 1 or d == 3:
        if player.x+40 > ghosts[g].x and dirs[0] == 1: ghosts[g].dir = 0
        if player.x+40 < ghosts[g].x and dirs[2] == 1: ghosts[g].dir = 2
    if d == 0 or d == 2:
        if player.y > ghosts[g].y and dirs[1] == 1 and not aboveCentre(ghosts[g]): ghosts[g].dir = 1
        if player.y < ghosts[g].y and dirs[3] == 1: ghosts[g].dir = 3

def followPlayer4(g, dirs):
    d = ghosts[g].dir
    if d == 1 or d == 3:
        if player.x-40 > ghosts[g].x and dirs[0] == 1: ghosts[g].dir = 0
        if player.x-40 < ghosts[g].x and dirs[2] == 1: ghosts[g].dir = 2
