...
def pubStatusData(client, freq=10, limit=100):
    ...
    for i in range(limit*freq):
        ...
        row = "{:s}, Cpu_temp : {:s}, Cpu_usuage : {:s}%, Using_mem : {:.2f}MB, Total_mem : {:.2f}MB".format(ti.strftime("%Y-%m-%d %H:%M:%S"),cpu_temp[-7:-1],cpu_use,mem_use,mem_total)
